	...
	for idx := range pieces {
		slices.SortFunc(
			pieces[idx].proofs, func(i, j *proof.Proof) int {
				return InputSort(i.Asset, j.Asset)
			},
		)
	}
}

// InputSort is a comparison function used to sort asset inputs by amount and
// then by script key. This is used to ensure that the inputs are sorted in a
// deterministic way before they are distributed to the outputs.
func InputSort(i, j asset.Asset) int {
	return cmp.Or(
...
func DistributeCoins(inputs []*proof.Proof, allocations []*Allocation,
	...
	for i, assetID := range uniqueAssetIDs {
		...
		// Before we create the virtual packet, we need to sort the
		// proofs by amount and then by script key. This is important to
		// ensure that the proofs are sorted in a deterministic way,
		// before we assign them to the inputs of the virtual packet.
